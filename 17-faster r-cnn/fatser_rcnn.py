...
class BananaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = torch.as_tensor([label], dtype=torch.int64)  # 类别从0开始
...
train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=4, collate_fn=lambda x: list(zip(*x)))
val_loader = DataLoader(val_dataset, batch_size=2, shuffle=False, num_workers=4, collate_fn=lambda x: list(zip(*x)))

# 4. 加载预训练的 Faster R-CNN 模型并调整
